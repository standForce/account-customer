...
public class AccountService {
    ...
    public Flux<AccountModel> getAccountList() {
        ...
        return accountRepository.findAll()
                .doOnError(error -> log.error("Error findAll account: " ,error))
    ...
    public Mono<AccountModel> getAccountById(int accountId) {
        ...
        return accountRepository.findById(accountId)
                .doOnError(error -> log.error("Error getAccountById account: " ,error))
                .map(accountMapper::getAccountModelFromEntity);
    }
    ...
    public Mono<AccountModel> saveAccount(Account account) {
        ...
        return accountRepository.save(account)
                .doOnError(error -> log.error("Error save account: " ,error))
                .map(accountMapper::getAccountModelFromEntity);
    }

    public Mono<AccountModel> updateAccount(int accountId, Account account) {
        log.info("accountId: {}, account: {}",accountId,account);
        return accountRepository.save(new Account(accountId, account.getCustomerId(), account.getNumber(), account.getType(), account.getBalance(), account.isStatus()))
